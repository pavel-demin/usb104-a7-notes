...
class source(gr.sync_block):
    ...
    def __init__(self, freq, rate, corr):
        ...
        self.send_command(1, 0, 0x00003C)
        self.send_command(1, 0, 0x000803)
        self.send_command(1, 0, 0x000800)
        self.send_command(1, 0, 0x000502)
        self.send_command(1, 0, 0x001421)
        self.send_command(1, 0, 0x000501)
        self.send_command(1, 0, 0x001431)
    ...
    def send_command(self, hi, lo, value):
        command = np.uint64((hi << 56) + (lo << 32) + value)
        self.device.write_data(command.tobytes())

    def set_freq(self, freq, corr):
        value = (1.0 + 1e-6 * corr) * freq
        self.send_command(0, 3, np.floor(value / 122.88e6 * (1 << 30) + 0.5))
        if value == 0:
            self.send_command(0, 4, 1)
        else:
            self.send_command(0, 4, 0)

    def set_rate(self, rate):
        if rate in source.rates:
            value = source.rates[rate]
            self.send_command(0, 2, value)
        else:
            raise ValueError(
